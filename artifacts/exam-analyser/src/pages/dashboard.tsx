        ...
        <div className="max-w-6xl mx-auto space-y-6 md:space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
            {STAT_CARDS.map(({ key, label, icon: Icon, gradient, shadow }) => (
              <div key={key} className={"relative overflow-hidden rounded-2xl bg-gradient-to-br " + gradient + " p-4 md:p-6 text-white shadow-lg " + shadow}>
          ...
          <div>
            ...
            {isLoading ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[1, 2, 3].map(i => <Skeleton key={i} className="h-52 w-full rounded-3xl" />)}
              </div>
                      ...
                      {hasData && totalGraded > 0 && (
                        <div className="px-5 pt-2 pb-3 flex gap-2 flex-wrap">
                          {(["EE", "ME", "AE", "BE"] as const).map(g => (
                            grades[g] > 0 && (
                              <span key={g} className={"text-xs px-2 py-0.5 rounded-full " + gradePillColor(g)}>
                                {g} · {grades[g]}
                              </span>
                            )
                          ))}
                      ...
                      {snap.topStudentName && (
                        <div className="px-5 py-2 border-t border-slate-50 flex items-center justify-between">
                          ...
                          {snap.topStudentGrade && (
                            <Badge className={"font-bold px-2 py-0.5 text-xs rounded-lg flex-shrink-0 " + gradeColor(snap.topStudentGrade)}>
                              {snap.topStudentGrade}
                            </Badge>
                          )}
                        </div>
