...
export default function FeeRemindersScreen() {
  ...
  if (!isAdmin) {
    return (
      <View style={[styles.container, { justifyContent: "center", alignItems: "center", padding: 32, gap: 10 }]}>
        <Ionicons name="lock-closed-outline" size={40} color={colors.mutedForeground} />
        <Text style={{ fontFamily: "Poppins_600SemiBold", fontSize: 15, color: colors.foreground, textAlign: "center" }}>
          Admin Only
        </Text>
        <Text style={{ fontFamily: "Poppins_400Regular", fontSize: 13, color: colors.mutedForeground, textAlign: "center" }}>
          Fee reminders can only be sent by an admin account.
        </Text>
      </View>
    );
  }
