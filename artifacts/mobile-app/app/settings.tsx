    ...
    <ScrollView style={s.container} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      ...
      <View style={s.section}>
        ...
        <View style={[s.row, { borderBottomWidth: 0 }]}>
          ...
          <Text style={[s.rowValue, { textTransform: "capitalize" }]}>{role}</Text>
        </View>
      </View>

      {/* ── School Administration (admin / principal / deputy only) ── */}
      {isStaff && (
        <View style={s.section}>
          <View style={s.sectionHeader}>
            <Ionicons name="shield-checkmark-outline" size={20} color={colors.primary} />
            <Text style={s.sectionTitle}>School Administration</Text>
          </View>
          <TouchableOpacity style={s.row} onPress={() => router.push("/admin/users")}>
            <Ionicons name="people-outline" size={18} color={colors.mutedForeground} style={{ marginRight: 10 }} />
            <Text style={[s.rowLabel, { flex: 1 }]}>Users & Roles</Text>
            {!isAdmin && <Text style={{ fontFamily: "Poppins_400Regular", fontSize: 11, color: colors.mutedForeground, marginRight: 6 }}>View only</Text>}
            <Ionicons name="chevron-forward" size={16} color={colors.mutedForeground} />
          </TouchableOpacity>
          <TouchableOpacity style={s.row} onPress={() => router.push("/admin/school-settings")}>
            <Ionicons name="school-outline" size={18} color={colors.mutedForeground} style={{ marginRight: 10 }} />
            <Text style={[s.rowLabel, { flex: 1 }]}>School Settings</Text>
            <Ionicons name="chevron-forward" size={16} color={colors.mutedForeground} />
          </TouchableOpacity>
          <TouchableOpacity style={[s.row, { borderBottomWidth: 0 }]} onPress={() => router.push("/admin/learning-areas")}>
            <Ionicons name="book-outline" size={18} color={colors.mutedForeground} style={{ marginRight: 10 }} />
            <Text style={[s.rowLabel, { flex: 1 }]}>Learning Areas</Text>
            <Ionicons name="chevron-forward" size={16} color={colors.mutedForeground} />
          </TouchableOpacity>
        </View>
      )}
      ...
      {isStaff && (
        <View style={s.section}>
          <TouchableOpacity style={[s.row, { borderBottomWidth: 0 }]} onPress={() => router.push("/admin/activity-log")}>
            <Ionicons name="time-outline" size={18} color={colors.mutedForeground} style={{ marginRight: 10 }} />
            <Text style={[s.rowLabel, { flex: 1 }]}>Activity Log</Text>
            <Ionicons name="chevron-forward" size={16} color={colors.mutedForeground} />
          </TouchableOpacity>
        </View>
      )}
