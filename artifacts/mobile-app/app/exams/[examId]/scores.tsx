...
export default function ScoresScreen() {
  ...
  const styles = StyleSheet.create({
    ...
    lockBanner: {
      ...
      borderRadius: colors.radius, backgroundColor: "#f0fdf4", borderWidth: 1, borderColor: "#bbf7d0",
    },
    lockBannerText: { fontFamily: "Poppins_500Medium", fontSize: 12, color: "#166534", flex: 1 },
    card: {
      backgroundColor: colors.card,
      marginHorizontal: 16,
      marginTop: 10,
  ...
  return (
    ...
    >
      ...
      {data.scoresApprovedAt && (
        <View style={styles.lockBanner}>
          <Ionicons name="lock-closed" size={15} color="#166534" />
          <Text style={styles.lockBannerText}>
            Scores are approved and locked. An admin, principal, or deputy can unapprove them from the Rankings screen to make corrections.
          </Text>
        </View>
      )}
      ...
      {data.rows.map((row) => (
        <View key={row.studentId} style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            {canWrite && !data.scoresApprovedAt && (
              <TouchableOpacity
