...
type OcrRow = {
  ...
  studentName: string;
  admissionNo: string;
  matchType: "exact_admission" | "exact_name" | "partial_name" | "unmatched";
  duplicateMatch: boolean;
  marks: OcrMark[];
};
type OcrResult = { examId: number; examName: string; className: string; scores: OcrRow[] };

const MATCH_BADGE: Record<OcrRow["matchType"], { label: string; bg: string; fg: string } | null> = {
  exact_admission: null, // the ideal case — no badge needed, keeps the review focused on what needs a look
  exact_name: { label: "Matched by name", bg: "#dbeafe", fg: "#1e40af" },
  partial_name: { label: "Partial match — verify", bg: "#fef3c7", fg: "#92400e" },
  unmatched: null, // already shown via the existing red "Not matched" text
};
...
export default function OcrUploadScreen() {
  ...
  const styles = StyleSheet.create({
    ...
    matchBadge: {
      paddingHorizontal: 8, paddingVertical: 3, borderRadius: 20, marginLeft: 8,
    },
    matchBadgeText: { fontFamily: "Poppins_600SemiBold", fontSize: 10 },
    duplicateBanner: {
      flexDirection: "row", alignItems: "center", gap: 6,
      backgroundColor: "#fee2e2", paddingHorizontal: 12, paddingVertical: 8,
      borderBottomWidth: 1, borderBottomColor: colors.border,
    },
    duplicateBannerText: { fontFamily: "Poppins_500Medium", fontSize: 11, color: "#991b1b", flex: 1 },
    markRow: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
  ...
  const handleSave = async () => {
    ...
    const duplicateRows = result.scores.filter(r => r.duplicateMatch);
    if (duplicateRows.length > 0) {
      Alert.alert(
        "Duplicate matches found",
        `${duplicateRows.length} row${duplicateRows.length > 1 ? "s" : ""} matched the same student as another row. Saving now may overwrite one of them with the other's marks. Save anyway?`,
        [
              ...
              {row.duplicateMatch && (
                <View style={styles.duplicateBanner}>
                  ...
                  <Text style={styles.duplicateBannerText}>
                    Same student as another row on this sheet — check names before saving.
                  </Text>
                </View>
              )}
              <View style={styles.studentHeader}>
                ...
                {badge && (
                  <View style={[styles.matchBadge, { backgroundColor: badge.bg }]}>
                    <Text style={[styles.matchBadgeText, { color: badge.fg }]}>{badge.label}</Text>
                  </View>
                )}
              </View>
              {row.marks.map((m) => {
