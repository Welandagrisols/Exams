...
router.get("/scores/:examId", async (req, res): Promise<void> => {
  ...
  const [exam] = await db
    .select({ id: examsTable.id, name: examsTable.name, classId: examsTable.classId, scoresApprovedAt: examsTable.scoresApprovedAt })
    .from(examsTable)
  ...
  res.json({ examId: exam.id, examName: exam.name, classId: exam.classId, scoresApprovedAt: exam.scoresApprovedAt ? new Date(exam.scoresApprovedAt).toISOString() : null, rows });
});
...
router.post("/scores", async (req, res): Promise<void> => {
  ...
  const [_examR] = await db.select({ classId: examsTable.classId, scoresApprovedAt: examsTable.scoresApprovedAt }).from(examsTable).where(eq(examsTable.id, examId));
  ...
  if (_examR.scoresApprovedAt) {
    res.status(409).json({ error: "Scores for this exam are approved and locked. An admin, principal, or deputy needs to unapprove them before edits can be made." });
    return;
  }
...
router.post("/scores/bulk", async (req, res): Promise<void> => {
  ...
  // RBAC: class teacher or staff only
  const [_bulkExamR] = await db.select({ classId: examsTable.classId, scoresApprovedAt: examsTable.scoresApprovedAt }).from(examsTable).where(eq(examsTable.id, parsedExamId));
  ...
  if (_bulkExamR.scoresApprovedAt) {
    res.status(409).json({ error: "Scores for this exam are approved and locked. An admin, principal, or deputy needs to unapprove them before edits can be made." });
    return;
  }
  ...
  logAudit(res.locals as AppLocals, "score.bulk_save", "exam", parsedExamId, { saved, skipped, studentCount: students.length });
