...
/**
 * Returns true ONLY for `admin`. Deliberately stricter than isStaff().
 * Reserved for the single most sensitive action in the system: granting or
 * revoking admin/principal/deputy access. Principals and deputies are staff
 * for day-to-day school administration (classes, exams, school settings,
 * learning areas) but must NOT be able to grant themselves or anyone else
 * elevated roles — that would let any staff account silently promote itself.
 */
export function isAdmin(locals: AppLocals): boolean {
