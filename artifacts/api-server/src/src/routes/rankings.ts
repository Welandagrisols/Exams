...
router.get("/rankings/:examId", async (req, res): Promise<void> => {
  ...
  // Standard competition ranking ("1224"): students tied on totalMarks share
  // the same rank, and the next distinct total's rank skips ahead by however
  // many students were tied before it (e.g. 1, 2, 2, 4 — not 1, 2, 2, 3).
  let currentRank = 0;
  let previousMarks: number | null = null;
  const ranked = results.map((r, i) => {
    if (r.totalMarks !== previousMarks) {
      currentRank = i + 1;
      previousMarks = r.totalMarks;
    }
