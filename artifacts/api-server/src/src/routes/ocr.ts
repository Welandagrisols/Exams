...
- Subject keys must match the subject names exactly as given above`;
  ...
  try {
    ...
    const enriched = (parsed.scores ?? []).map((row: any) => {
      let matchType: "exact_admission" | "exact_name" | "partial_name" | "unmatched" = "unmatched";
      ...
      if (student) matchType = "exact_admission";
      ...
      if (!student && row.studentName) {
        ...
        if (student) matchType = "exact_name";
      }
      ...
      if (!student && row.studentName) {
        ...
        if (rowWords.length >= 2) {
          ...
          if (student) matchType = "partial_name";
        }
      }
    ...
    // Flag rows that resolved to the SAME student — e.g. two similarly-spelled
    // names on the sheet both matching one DB record. Left unflagged, saving
    // would silently overwrite one row's marks with the other's.
    ...
    for (const row of enriched) {
      (row as any).duplicateMatch = row.studentId != null && (studentIdCounts.get(row.studentId) ?? 0) > 1;
    }
