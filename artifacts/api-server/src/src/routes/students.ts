...
router.post("/students", async (req, res): Promise<void> => {
  ...
  let student;
  try {
    [student] = await db.insert(studentsTable).values(parsed.data).returning();
  } catch (err: any) {
    if (err?.code === "23505") { // Postgres unique_violation
      res.status(409).json({ error: `Admission number "${parsed.data.admissionNo}" is already in use by another student.` });
      return;
    }
    throw err;
  }
  logAudit(res.locals as AppLocals, "student.create", "student", student.id, { name: student.name, admissionNo: student.admissionNo, classId: student.classId });
  const [row] = await db
    .select(STUDENT_SELECT)
    .from(studentsTable)
    .leftJoin(classesTable, eq(classesTable.id, studentsTable.classId))
...
router.patch("/students/:id", async (req, res): Promise<void> => {
  ...
  logAudit(res.locals as AppLocals, "student.update", "student", params.data.id, { changes: parsed.data });
  const [row] = await db
    .select(STUDENT_SELECT)
    .from(studentsTable)
    .leftJoin(classesTable, eq(classesTable.id, studentsTable.classId))
...
router.delete("/students/:id", async (req, res): Promise<void> => {
  ...
  // RBAC: fetch student's class, then check
  const [_studDel] = await db.select({ classId: studentsTable.classId, name: studentsTable.name }).from(studentsTable).where(eq(studentsTable.id, params.data.id));
  ...
  logAudit(res.locals as AppLocals, "student.delete", "student", params.data.id, { name: _studDel.name });
  res.sendStatus(204);
});
